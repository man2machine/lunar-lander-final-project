...
ENGINE_FORCE_LIMITS = np.array([0.2, 0.2, 5]) * GRAVITY * LANDER_MASS
LANDER_CENTER = np.array([0, 0])
LANDER_MOMENT_I = (1/12) * W*H*(W*W + H*H) * LANDER_MASS
...
class LunarLander(gym.Env):
    ...
    def step(self, action, apply_anim=True):
        ...
        if apply_anim:
            ...
            #     p = self._create_particle(1, force_loc_world[0], force_loc_world[1], u_i * 5)
            #     impulse = -f
            #     #print(impulse, u_i)
...
def heuristic(x):
    ...
    angle_todo = (angle_targ - x[4]) * 0.5 - (x[5]) * 1.0
