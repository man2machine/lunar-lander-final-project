...
class LunarLander(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        for a, d in zip([action[1], action[2]], [-1, 1]):
            ...
            ox = side[0] * (direction * 4/SCALE)
            oy =  -side[1] * (direction * 4/SCALE)
            impulse_pos = (self.lander.position[0] + ox - tip[0] * 17/SCALE,
                            self.lander.position[1] + oy + tip[1] * SIDE_ENGINE_HEIGHT/SCALE)
            p = self._create_particle(0.05, impulse_pos[0], impulse_pos[1], s_power)
        ...
        if self.game_over or abs(state[0]) >= 1.0:
            ...
            reward = 100 if abs(state[5]) < np.pi/4 else -100
...
def heuristic(env, s):
    ...
    u_u = hover_todo * 20
    if angle_todo < 0:
        u_r = abs(angle_todo) * 20
        u_l = 0
    else:
        u_l = abs(angle_todo) * 20
        u_r = 0
